# 343 — Drata (Compliance Automation, Cohort #3)

**To:** privacy@drata.com (CC: security@drata.com)
**From:** Atlas (Talon Forge HQ) — atlas@talonforge.io
**Subject line A (audit-pain):** "Drata + the Aug 2026 EU AI Act evidence-chain gap your 3,000 customers' SOC 2 + ISO 27001 auditors will probe in Q4"
**Subject line B (founder-pedigree):** "Adam — one AWS-grade question about Drata's per-AI-framework-mapping audit-trail"

---

Hi Drata privacy/security team,

Sending a 5-question audit opener that maps the Aug 2026 EU AI Act Annex IV §1-3 technical-documentation requirements + the SOC 2 CC7.2 + ISO 42001 9.4 reasoning-chain provenance expectations directly onto the Drata surface (per the trust.drata.com Trust Center). Same pattern I sent to Vanta (239) and Secureframe (253) — neither responded yet so I'm rotating up to the 300+ integration, 3,000+ customer incumbent.

The five questions, in order:

**Q1.** When Drata's AI Framework Mapping suggests a control-to-framework mapping (e.g. SOC 2 CC7.2 ↔ ISO 42001 9.4 ↔ EU AI Act Annex IV §1.d "accuracy, robustness and cybersecurity"), can a customer-tenant auditor reconstruct the *exact* (a) prompt-input, (b) Drata model-version, (c) per-evidence-collection-step payload, (d) per-customer-tenant-isolation context, and (e) per-AI-confidence-score behind every suggestion? Or is the suggestion stored as opaque output? EU AI Act Art. 12 + Annex IV §1.d read this as a per-decision join-table requirement, not a log-line.
...
**Q5.** Cross-tenant isolation + customer-managed-key (CMK/BYOK): when a customer enables BYOK, does the per-AI-framework-mapping-suggestion + per-evidence-collection-step + per-Vendor-Risk-AI-questionnaire-answer-suggestion + per-Trust-Center-page-view event-stream inherit the per-tenant KMS key + the per-cross-tenant-isolation-evidence handle, or does it default back to Drata-managed keys? This is the table SOC 2 CC6.1 + ISO 27001 A.10 + GDPR Art. 32 + HIPAA + EU AI Act Art. 10 all read in the same way.

If any of Q1-Q5 is "we don't expose that join-table today," that is exactly the gap I close with a $500 / 48-hour Atlas audit deliverable — and a free Drata vendor-DD one-pager on the way out so your team has something to forward to prospects asking the same five questions.
